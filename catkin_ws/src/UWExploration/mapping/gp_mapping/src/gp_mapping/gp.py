...
class RGP(ExactGP):
    ...
    def plot(self, fname, n=100, n_contours=50):
        ...
        fig, ax = plt.subplots(3, 1, sharex=True, sharey=True,figsize=(5, 9), constrained_layout=True)
        ...
        #change
        cb0 = fig.colorbar(cr, ax=ax[0], fraction=0.04, pad=0.02)
        cb1 = fig.colorbar(cm, ax=ax[1], fraction=0.04, pad=0.02)
        cb2 = fig.colorbar(cv, ax=ax[2], fraction=0.04, pad=0.02)
...
class SVGP(VariationalGP):
    ...
    def plot(self, inputs, targets, fname, n=80, n_contours=50, track=None):
        ...
        fig, ax = plt.subplots(3, 1, sharex=True, sharey=True,figsize=(9, 15))
        ...
        for cb in (cb0, cb1, cb2):
            cb.ax.yaxis.set_major_locator(MaxNLocator(nbins=5))  # 色条最多5个刻度
        ...
        for a in ax:
            a.xaxis.set_major_locator(MaxNLocator(nbins=4))  # x轴刻度压缩
            a.yaxis.set_major_locator(MaxNLocator(nbins=6))  # y轴刻度压缩
            a.tick_params(labelsize=9)

        # formatting
        ax[0].set_aspect('equal')
        ax[0].set_title('Raw data', fontsize=10)
        ax[0].set_ylabel('$y~[m]$',fontsize=10)
        ax[1].set_aspect('equal')
        ax[1].set_title('Mean',fontsize=10)
        ax[1].set_ylabel('$y~[m]$',fontsize=10)
        ax[2].set_aspect('equal')
        ax[2].set_title('Variance',fontsize=10)
        ax[2].set_xlabel('$x~[m]$',fontsize=10)
        ax[2].set_ylabel('$y~[m]$',fontsize=10)
        ax[2].set_title('Variance', pad=12)
